...
public class tuningChart extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tuning_chart);
...
//        scatter.xScale().tick
    ...
    private List<DataEntry> getLineData() {
        ...
        for(int i = 0; i < 10; i++){
            data.add(new ValueDataEntry(i, ((m * i) + b)));
        }
        return data;
    }
    ...
    private List<DataEntry> getMarkerData(){
        ...
        try{
            ...
            FileInputStream fis = new FileInputStream("file:///android_asset/config.txt");
            BufferedReader bfr = new BufferedReader(new InputStreamReader(fis));

            String str = "";
            String rpm = "";

            m = Double.valueOf(bfr.readLine());
            b = Double.valueOf(bfr.readLine());

            while ((str = bfr.readLine()) != null) {
                Log.e("Data Value", str);
                data.add(new ValueDataEntry(Integer.valueOf(rpm),Integer.valueOf(str)));
            }
            bfr.close();
        }
        catch(Exception e){ Log.e("Error", "Something went wrong with the file read in");}

        return data;
    }

    private List<DataEntry>getRPMData() {
        List<DataEntry> data = new ArrayList<>();
